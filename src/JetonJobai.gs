/**
 * JetonJobai.gs — jeton OAuth du projet GCP « jobai » (hubperso) pour Tasks & Calendar (ADR-0041).
 *
 ...
 * « jobai » ; le moteur obtient un jeton OAuth de CE projet (client OAuth de Marc, consentement
 ...
 *  - Le `client_secret` ne transite QUE vers `JOBAI_URL_JETON` — seul point `UrlFetchApp.fetch`
 ...
 *  - Le callback `?jobai=1` (doGet, WebApp.gs) vérifie `state` en comparaison constante : l'URL
 ...
 *
 * Mise en service (une fois, pas-à-pas : docs/JOBAI.md) : Marc crée un client OAuth « Web » dans
 * jobai, pose `DriveAI_JOBAI_CLIENT_ID` + `DriveAI_JOBAI_CLIENT_SECRET`, exécute
 * `lierCompteJobai` (ce fichier) et clique l'URL de consentement que la fonction affiche.
 */

var JOBAI_URL_JETON = 'https://oauth2.googleapis.com/token';
var JOBAI_URL_CONSENTEMENT = 'https://accounts.google.com/o/oauth2/v2/auth';
...
var JOBAI_SCOPES = 'https://www.googleapis.com/auth/tasks https://www.googleapis.com/auth/calendar.events';
...
var JOBAI_MARGE_EXPIRATION_MS = 7 * 60 * 1000;
// Péremption du state anti-CSRF (revue sécurité A) : un `lierCompteJobai` abandonné ne doit pas
...
var JOBAI_STATE_MAX_AGE_MS = 60 * 60 * 1000;

/**
 * Rend un access token VALIDE du projet jobai, ou `null` — ÉCHEC FERMÉ (l'appelant suspend via la
 * mécanique config-api, il ne re-tente jamais en boucle). Cache en Script Property
 * (`DriveAI_JOBAI_ACCES` = « expirationMs|token »), rafraîchi via REST quand périmé.
 * @return {?string}
 */
function jetonJobai_() {
  var props = null;
  try { props = PropertiesService.getScriptProperties(); } catch (e) { return null; }

  var enCache = null;
  try { enCache = jetonCacheValide_(props.getProperty('DriveAI_JOBAI_ACCES'), Date.now()); } catch (e) { }
  if (enCache) return enCache;

  var clientId = '', clientSecret = '', refresh = '';
  try {
    clientId = props.getProperty('DriveAI_JOBAI_CLIENT_ID') || '';
    clientSecret = props.getProperty('DriveAI_JOBAI_CLIENT_SECRET') || '';
    refresh = props.getProperty('DriveAI_JOBAI_REFRESH') || '';
  } catch (e) { return null; }
  if (!clientId || !clientSecret || !refresh) return null; // jamais lié (ou purgé) : docs/JOBAI.md

  var rep;
  try {
    rep = UrlFetchApp.fetch(JOBAI_URL_JETON, {
      method: 'post',
  ...
  var resultat = analyserReponseJetonJobai_(rep.getResponseCode(), rep.getContentText(), Date.now());
  if (resultat.revoque) {
    ...
    try {
      if (props.getProperty('DriveAI_JOBAI_REFRESH') === refresh) {
        props.deleteProperty('DriveAI_JOBAI_REFRESH');
        props.deleteProperty('DriveAI_JOBAI_ACCES');
        journalErreur_('Jobai', 'Consentement jobai RÉVOQUÉ (invalid_grant) — re-lier le compte : ' +
          'exécuter lierCompteJobai (JetonJobai.gs) et suivre docs/JOBAI.md.');
      }
    } catch (e) { }
    return null;
  }
  ...
  try { props.setProperty('DriveAI_JOBAI_ACCES', resultat.expireMs + '|' + resultat.jeton); } catch (e) { }
  return resultat.jeton;
}

/**
 * Extrait le token d'un cache « expirationMs|token » s'il est encore valide (marge
 * `JOBAI_MARGE_EXPIRATION_MS` incluse). PURE (testée).
 * @param {?string} brut
...
function jetonCacheValide_(brut, maintenantMs) {
  ...
  return maintenantMs < expireMs - JOBAI_MARGE_EXPIRATION_MS ? jeton : null;
}

/**
 * État de la LIAISON jobai (credentials présents ?), pour distinguer « jamais lié / révoqué »
 ...
 */
function etatLiaisonJobai_() {
  try {
    var props = PropertiesService.getScriptProperties();
    return (props.getProperty('DriveAI_JOBAI_CLIENT_ID') && props.getProperty('DriveAI_JOBAI_CLIENT_SECRET') &&
      props.getProperty('DriveAI_JOBAI_REFRESH')) ? 'present' : 'absent';
  } catch (e) { return 'inconnu'; }
}

/**
 * Message de panne quand `jetonJobai_()` rend null — HONNÊTE sur la cause (revue quotas F2).
 ...
 * @return {string}
 */
function messageJetonJobaiIndisponible_() {
  return etatLiaisonJobai_() === 'absent'
    ? 'compte jobai non lié — exécuter lierCompteJobai (docs/JOBAI.md)'
    : 'jeton jobai momentanément indisponible (échec transitoire du refresh OAuth)';
}
 ...
 */
function purgerCacheJetonJobai_() {
  try { PropertiesService.getScriptProperties().deleteProperty('DriveAI_JOBAI_ACCES'); } catch (e) { }
}
 ...
 */
function analyserReponseJetonJobai_(code, corps, maintenantMs) {
  var j = null;
...
/**
 * À EXÉCUTER PAR MARC dans l'éditeur (ouvre `JetonJobai.gs` → `lierCompteJobai` → Exécuter),
 * APRÈS avoir posé `DriveAI_JOBAI_CLIENT_ID` et `DriveAI_JOBAI_CLIENT_SECRET` (docs/JOBAI.md).
 * Affiche DEUX choses dans le journal d'exécution : (1) l'URI de redirection à déclarer dans le
 * client OAuth jobai (EXACTEMENT, à l'octet près), (2) l'URL de consentement à ouvrir.
 ...
 */
function lierCompteJobai() {
  var props = PropertiesService.getScriptProperties();
  var clientId = props.getProperty('DriveAI_JOBAI_CLIENT_ID');
  if (!clientId || !props.getProperty('DriveAI_JOBAI_CLIENT_SECRET')) {
    throw new Error('Poser d\'abord DriveAI_JOBAI_CLIENT_ID et DriveAI_JOBAI_CLIENT_SECRET ' +
      'dans les Script Properties (docs/JOBAI.md, étapes 1-2).');
  }
  ...
  var rappel = props.getProperty('DriveAI_JOBAI_REDIRECT') || ScriptApp.getService().getUrl();
  if (!rappel) throw new Error('Web app non déployée : impossible de construire l\'URI de rappel.');
  props.setProperty('DriveAI_JOBAI_REDIRECT', rappel);
  var state = Utilities.getUuid();
  // Horodaté : le callback refuse un state plus vieux que JOBAI_STATE_MAX_AGE_MS (revue sécurité
  // A — une liaison abandonnée ne laisse pas un state valable à vie). L'URL ne porte que l'UUID.
  props.setProperty('DriveAI_JOBAI_STATE', state + '|' + Date.now());
  var url = urlConsentementJobai_(clientId, rappel, state);
  Logger.log('1) URI de redirection à déclarer dans le client OAuth jobai (exactement) :\n' + rappel +
 ...
 * @return {string}
 */
function urlConsentementJobai_(clientId, redirectUri, state) {
  return JOBAI_URL_CONSENTEMENT +
    '?client_id=' + encodeURIComponent(clientId) +
    '&redirect_uri=' + encodeURIComponent(redirectUri) +
    '&response_type=code' +
    '&scope=' + encodeURIComponent(JOBAI_SCOPES) +
    '&access_type=offline' +
    '&prompt=consent' +
    '&state=' + encodeURIComponent(state);
}

/* ---------- Callback (doGet ?jobai=1) ---------- */
 ...
 * @return {HtmlOutput}
 */
function traiterCallbackJobai_(params) {
  var ok = false;
  try { ok = echangerCodeJobai_(params); }
  catch (e) {
    try { journalErreur_('Jobai', 'Callback de liaison en échec : ' + e); } catch (e2) { }
  }
  return HtmlService.createHtmlOutput(ok
    ? '<p>✅ Compte jobai lié — DriveAI passe désormais par ton projet pour Tasks &amp; Calendar. ' +
      'Tu peux fermer cet onglet (reprise automatique au prochain tick).</p>'
    : '<p>❌ Liaison non aboutie. Relance <b>lierCompteJobai</b> (fichier JetonJobai.gs) ' +
      'et suis docs/JOBAI.md.</p>');
}
 ...
 */
function echangerCodeJobai_(params) {
  var props = PropertiesService.getScriptProperties();
  var code = validerCallbackJobai_(params, props.getProperty('DriveAI_JOBAI_STATE'), Date.now());
  if (!code) return false; // refus MUET (state absent/inconnu/périmé) — voir traiterCallbackJobai_
  var clientId = props.getProperty('DriveAI_JOBAI_CLIENT_ID');
  var clientSecret = props.getProperty('DriveAI_JOBAI_CLIENT_SECRET');
  var rappel = props.getProperty('DriveAI_JOBAI_REDIRECT'); // celle du consentement, à l'octet près
  if (!clientId || !clientSecret || !rappel) return false;

  var rep = UrlFetchApp.fetch(JOBAI_URL_JETON, {
    method: 'post',
  ...
  if (rep.getResponseCode() !== 200 || !j || typeof j.refresh_token !== 'string' || !j.refresh_token) {
    journalErreur_('Jobai', 'Échange du code refusé (HTTP ' + rep.getResponseCode() +
      ') — relancer lierCompteJobai (docs/JOBAI.md).');
    return false;
  }
  ...
  if (!scopesJobaiComplets_(j.scope)) {
    journalErreur_('Jobai', 'Liaison REFUSÉE : autorisations incomplètes au consentement — ' +
      'relancer lierCompteJobai et COCHER les deux autorisations (Tasks ET Agenda).');
    return false;
  }
  props.setProperty('DriveAI_JOBAI_REFRESH', j.refresh_token);
  if (typeof j.access_token === 'string' && j.access_token && Number(j.expires_in) > 0) {
    props.setProperty('DriveAI_JOBAI_ACCES', (Date.now() + Number(j.expires_in) * 1000) + '|' + j.access_token);
  }
  props.deleteProperty('DriveAI_JOBAI_STATE'); // state à usage UNIQUE
  journalInfo_('Jobai', 'Compte jobai lié — Tasks & Calendar passent par le projet jobai ' +
    '(reprise des intentions au prochain tick, via la sonde config-api).');
  return true;
}

/**
 * Valide les paramètres du callback : action `jobai=1`, `code` présent, `state` identique à
 * l'UUID généré par `lierCompteJobai` (comparaison CONSTANTE) et pas plus vieux que
 * `JOBAI_STATE_MAX_AGE_MS` (revue sécurité A). PURE (testée).
 * @param {Object} params
 * @param {?string} stateAttendu  Property « uuid|poseMs »
 * @param {number} maintenantMs
 * @return {string} le code d'autorisation, ou '' (refus).
 */
function validerCallbackJobai_(params, stateAttendu, maintenantMs) {
  if (!params || params.jobai !== '1') return '';
  ...
  if (!poseMs || maintenantMs - poseMs > JOBAI_STATE_MAX_AGE_MS) return ''; // state périmé
 ...
 * PURE (testée). Cf. le refus de liaison dans `echangerCodeJobai_` (consentement granulaire).
 * @param {*} scopes
 * @return {boolean}
 */
function scopesJobaiComplets_(scopes) {
