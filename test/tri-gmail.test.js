 ...
 * l'orchestration est vérifiée sur : idempotence sans chargement, attente des intentions,
 * écritures bornées (libellés existants seuls, jamais de création), archivage réversible.
 */
...
test('heuristiquePhishing_ : PJ risquée OU (urgence ET identifiants) — étroit, calibré', () => {
  assert.strictEqual(ctxPur.heuristiquePhishing_('Bonjour', ['facture.pdf', 'doc.zip']), true);
  assert.strictEqual(ctxPur.heuristiquePhishing_('URGENT : vérifiez votre compte', []), true);
...
test('parserMiniCategorie_ : catégorie acceptée SEULEMENT si exactement dans la liste', () => {
...
function ctxTri(opts) {
  ...
  for (const n of ['Finance', 'Abonnements', 'À vérifier', '⚠️ Suspect', '⏰ À traiter']) {
    labelObjs[n] = { getName: () => n, addToThread: (t) => calls.labels.push({ label: n, fil: t.__id }) };
  }
  c.GmailApp = {
...
function filMock(calls, { id, ts, dernierMsgId, expediteur, sujet, nonLu, unsubscribe }) {
  return {
    __id: id,
    getId: () => id,
...
test('tri : fil déjà trié dans cet état → sauté SANS charger les messages (clé fil|ts)', () => {
  const pre = {};
  const { c, calls } = (() => {
    const r = ctxTri({ index: { 'tri|F1|1000': true }, fils: [] });
...
test('tri : le dernier message PAS ENCORE analysé par les intentions → on ATTEND (clé non consommée)', () => {
  ...
  assert.strictEqual(calls.index['tri|F2|2000'], undefined); // re-tenté au prochain tick
});
...
test('tri : fil lu + catégorie sûre → libellé posé + archivé + indexé « trié »', () => {
  ...
  assert.deepStrictEqual(plain(calls.ajouts), [{ cle: 'tri|F3|3000', statut: 'trié' }]);
});
...
test('tri : promo déterministe (List-Unsubscribe) NON LUE → archivée ; la même en zone protégée → JAMAIS', () => {
...
test('tri : catégorie LLM inconnue → À vérifier, pas d\'archivage, pas d\'apprentissage', () => {
  ...
  assert.deepStrictEqual(calls.archives, []);
  assert.deepStrictEqual(plain(calls.ajouts), [{ cle: 'tri|F7|7000', statut: 'tri-a-verifier' }]);
});
...
/* ---------- scan ARRIÈRE (rattrapage du stock, curseur de date persistant) ---------- */

test('rattrapage : lot complet SANS attente → le curseur de date avance (jamais un offset)', () => {
  const { c, calls } = ctxTri({ index: { 'intention|MA1': true, 'intention|MA2': true }, props: { DriveAI_TRI_RATTRAPAGE: '' } });
  delete calls.props.DriveAI_TRI_RATTRAPAGE;
  const anciens = [
    filMock(calls, { id: 'A1', ts: new Date('2026-06-10T10:00:00Z').getTime(), dernierMsgId: 'MA1', expediteur: 'a@b.c', sujet: 's1' }),
    filMock(calls, { id: 'A2', ts: new Date('2026-06-08T10:00:00Z').getTime(), dernierMsgId: 'MA2', expediteur: 'a@b.c', sujet: 's2' }),
  ];
  let appels = 0;
  ...
  c.trierFilsGmail_(() => false);
  assert.strictEqual(calls.ajouts.length, 2);
  assert.strictEqual(calls.props.DriveAI_TRI_AVANT, '2026/06/08'); // date ABSOLUE du plus ancien
  assert.strictEqual(calls.props.DriveAI_TRI_RATTRAPAGE, 'terminé'); // recherche suivante vide → figé
});

test('rattrapage : un fil du lot ATTEND les intentions → curseur INCHANGÉ (le lot sera re-couvert)', () => {
  ...
  c.GmailApp.search = () => (appels++ === 1 ? anciens : []);
  c.trierFilsGmail_(() => false);
  assert.strictEqual(calls.props.DriveAI_TRI_AVANT, undefined);       // pas d'avance
...
test('fil malade → compté puis ABANDONNÉ après 3 essais (une seule annonce, plus jamais rechargé)', () => {
  const { c, calls } = ctxTri({ echecs: {} });
  const malade = { getId: () => 'MAL', getLastMessageDate: () => { throw new Error('boom'); } };
...
test('fil où MARC a répondu en dernier → catégorisé sur le message de l\'AUTRE, jamais appris sur sa propre adresse', () => {
  ...
  c.emailAlerte_ = () => 'marc.richard4@gmail.com';
  ...
  const fil = {
    __id: 'F9', getId: () => 'F9', getLastMessageDate: () => new Date(900), isUnread: () => false,
    getLabels: () => [],
    getMessages: () => [
