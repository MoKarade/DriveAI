...
test('indexAjouter_ : n\'écrit que les 9 colonnes métadonnées (aucun corps de doc)', () => {
  ...
  ctx.indexAjouter_('m|0|passeport.pdf|999', {
    ...
    fileId: 'ID_DRIVE_ABCDEFGHIJKLMNOP', // C49-16 — identifiant Drive, jamais un contenu
  }, 'HASH_ABCDEF0123');
  ...
  // ⚠️ 8 → 9 le 21/09/2026 (C49-16), et cette garde a TIRÉ pour l'exiger : elle a rougi sur la
  // colonne neuve et forcé à justifier qu'un `fileId` est bien une métadonnée. Il l'est —
  // l'ADR-0007 §2 range `Fichier` parmi les métadonnées légitimes, et cet identifiant était
  // DÉJÀ persisté dans la clé pour quatre préfixes sur cinq. Ce qui change n'est pas la nature
  // de la donnée, c'est qu'on cesse de la perdre pour le cinquième (les pièces jointes Gmail).
  // Un test de LIMITE s'INVERSE avec son histoire, il ne se supprime pas (leçon §9).
  assert.strictEqual(row.length, 9, '9 colonnes métadonnées, ni plus ni moins');
  ...
  // ⚠️ Le fileId est un IDENTIFIANT, jamais un texte libre : un champ qui accepterait n'importe
  // quelle chaîne serait un canal de fuite de plus, dans la colonne la moins relue.
  assert.strictEqual(row[8], 'ID_DRIVE_ABCDEFGHIJKLMNOP'); // FileId (C49-16)
...
test('indexAjouter_ : empreinte absente → cellule vide (jamais de fuite de contenu à la place)', () => {
  ...
  assert.strictEqual(row[8], ''); // pas de fileId → '' (jamais un repli deviné)
});
