...
| 6 | **Correction via mail → mini-formulaire** (cas « Inconnu » + entités à valider) | Contrôle | M-L | [0003](adr/0003-controle-correction.md) | 🟦 formulaire Google (find-or-create) + lecture des réponses → corrections (few-shot), lien au résumé hebdo ✅ · **⚠️ nouveau scope `forms` = ré-autorisation de Marc au prochain déploiement** · déplacement du fichier corrigé = à suivre |
