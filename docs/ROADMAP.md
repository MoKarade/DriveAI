...
| 12 | **Historique Gmail complet** — ancre FIXE (`before:<ancre>` figée) + offset, terminé par passe de VÉRIFICATION propre, toutes les vieilles PJ classées | Mails | M | [0010](adr/0010-mails-historique-visibilite.md) | 🟦 codé (design v2, durci par 2 vérifs adversariales — le curseur rétrograde v1 perdait des PJ + plateau infini) : offset n'avance que sur page COMPLÈTE (rejeu idempotent), garde-temps/plafond PAR PJ (2 inédites/run — quota runtime, le vivant prioritaire), fil en erreur revisité puis abandonné à 3 essais, « terminé » seulement quand une passe complète ne collecte plus rien ✅ |
