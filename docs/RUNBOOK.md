...
2. **Cause probable A — déclencheur désactivé** par Google (trop d'échecs). **Auto-réparé** par le **chien de garde** (2ᵉ déclencheur `chienDeGarde`, toutes les 30 min, ADR-0004) : il détecte le silence (heartbeat `DriveAI_LAST_TICK` périmé > 45 min), **ré-installe seul** le déclencheur principal, et journalise « exécuter `installerTrigger` » si l'auto-réparation ne suffit pas — **aucun mail** (décision Marc 2026-07-06, puis C28-75 : plus aucun mail du moteur). Remède manuel (si alerté) inchangé : **script.google.com → projet DriveAI → exécuter `installerTrigger`**. Cas résiduel : si Google désactive *tous* les déclencheurs (watchdog inclus), l'app (onglet Santé) et le MCP `etat_moteur` servent de second filet, sinon un clic.
