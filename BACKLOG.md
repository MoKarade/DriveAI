...
| C26-07 | **PREUVE** : `src/DryRunV2.gs` — échantillon RÉEL stratifié par domaine (persisté, reproductible), exécute le vrai pipeline v2 (`classifierDeuxPasses_` + `planRoutageV2_` PUR), écrit l'avant/après dans l'onglet Sheet `DryRunV2` — ZÉRO mutation Drive (tripwire statique testé empiriquement). Interrupteur DÉDIÉ `CONFIG.DRYRUN_V2_ACTIF` (jamais `ANALYSE_V2`). Revue flotte (6 agents) : budget-temps aligné sur le coût-temps Sonnet ×2, collecte découplée du sous-budget LLM, chemin multi-niveaux, encodage compact anti-dépassement Property, tripwire étendu. +36 tests (382 → 418). `docs/RUNBOOK.md` : checklist d'allumage | 🟦 **run réel LANCÉ** (feu vert Marc 2026-07-08, `DRYRUN_V2_ACTIF: true`) — l'échantillon (~100 docs, ~3-6 $) se remplit dans l'onglet Sheet `DryRunV2` au fil des ticks → validation Marc, puis C26-08 |
