...
| C28-23 | **Refonte UI « style Google » complet + Agenda clone GCal** (demande Marc 2026-07-15, capture Google Agenda sombre fournie : « je veux que l'agenda ressemble exactement à ça et que le style de l'app complète épouse entièrement ce style google ») | Cadrage par 12 questions (réponses Marc 15/07) : sombre SEUL (toggle supprimé) ; sidebar Google PARTOUT (☰, + Créer, mini-calendrier, 6 sections) + barre haute Material ; SANS nouveau scope (agenda principal + tâches, liste simplifiée) ; un seul fuseau ; vue par défaut Semaine ; sélecteur Jour+Semaine+Mois (Jour = nouveau) ; couleurs par TYPE ; page d'entrée = cockpit v4 ; clic créneau → Créer pré-rempli ; clic événement → popover GCal ; mobile Semaine = 3 jours glissants + barre basse conservée ; réglages → menu avatar | 🧭 cadrage — prompt NotebookLM livré (1 923 car.), en attente du plan |
