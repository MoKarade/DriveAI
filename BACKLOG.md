...
| C28-20 | **Zéro configuration** (demande Marc 2026-07-13 : « je veux rien, juste mon compte Google ») | Cadrage : l'écran Configuration (spreadsheetId + URL/secret web app en localStorage) doit disparaître. Dépôt PUBLIC ⇒ rien en dur dans le bundle : proposition soumise à NotebookLM — `api/config.ts` servi par le BFF aux seules sessions valides (cookie déchiffrable) + verrou d'identité `ALLOWED_EMAIL` au callback (sinon n'importe quel compte Google obtiendrait le secret), env Vercel SPREADSHEET_ID/WEBAPP_URL/WEBAPP_SECRET/ALLOWED_EMAIL, mock E2E inchangé, session.test.ts intact | 🧭 cadrage — prompt NotebookLM livré (1 963 car.), en attente du plan |
